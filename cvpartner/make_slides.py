...
def make_ppt_from_year_in_review(projects_worked_on: Dict[str, list[ProjectExperienceExpanded]],
    ...
    try:
        ...
        all_names = list(set(projects_worked_on.keys() | new_courses.keys() | new_certifications.keys() | new_presentations.keys() | new_honors_and_awards.keys()))

        # loop over all people
        for name in all_names:
            if name in projects_worked_on:
                add_experience_slide(prs, bullet_slide_layout, name, projects_worked_on[name])

            if name in new_courses:
                add_course_slide(prs, bullet_slide_layout, name, new_courses[name])

            if name in new_certifications:
                add_certification_slide(prs, bullet_slide_layout, name, new_certifications[name])

            if name in new_presentations:
                add_presentation_slide(prs, bullet_slide_layout, name, new_presentations[name])

            if name in new_honors_and_awards:
                add_honors_and_awards_slide(prs, bullet_slide_layout, name, new_honors_and_awards[name])

        prs.save(f'presentation_{department_name}_{datetime.datetime.now().hour}{datetime.datetime.now().minute}.pptx')
        return True
